...
public:
    ...
    template<typename T1>
    friend matrix<T1> multiplication(matrix<T1> const& mat1, matrix<T1> const& mat2);
...
//----------------------------------------------------------------------------------
template <typename T>
void parallel_matrix_multiplication(int N, std::vector<T> mat1, std::vector<T> mat2, std::vector<T> & result)
{

    auto mul = [&](int const& b, int const& t)
    {
        std::vector<T> temp;
        temp.resize((t - b ) * N);
        for(int i{b}; i < t; i++)
    ...
    std::future<std::vector<double>> ghu;
    auto future1 = std::async(std::launch::async, mul, 0, k);
    auto future2 = std::async(std::launch::async, mul, k, N);
    auto r1 = future1.get();
    auto r2 = future2.get();
...
template<typename T>
matrix<T>  multiplication(matrix<T> const& mat1, matrix<T> const& mat2)
{
    matrix<T> r{mat1.dim};
    parallel_matrix_multiplication(mat1.dim, mat1.data, mat2.data, r.data);
    return r;
}

template<typename T>
void test(matrix<T> mat1, matrix<T> mat2)
{
    matrix<T> temp1 = mat1 * mat2;
    matrix<T> temp2 = multiplication(mat1, mat2);

    if(!check(temp1, temp2)){std::cout << "Error in matrix multiplication.";};
}