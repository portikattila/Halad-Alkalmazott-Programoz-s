...
int main(int, char**)
{
    ...
    double r = newton([](double x){ return x*x - 612.0; },
                    [](double x){ return 2.0 * x; },
                    [](double x1, double x0 ){ return std::abs(x1 - x0) > 1e-6; }, 1.); //abszolút eltérés figyelése
    //müködés ellenőrzése
    ...
    double r2 = newton([](double x){ return x*x - num; },
                    [](double x){ return 2.0 * x; },
                    [](double x1, double x0 ){ return std::abs(x1 - x0) > 1e-6; }, x0);
    std::cout << r2;
