...
public:
    ...
    template<typename T1>
    friend void set_dim(matrix<T1>& mat);

    template<typename T1>
    friend void add_element(matrix<T1>& mat, T1 v);
...
template<typename T>
void set_dim(matrix<T>& mat)
{
    mat.dim = sqrt(mat.size());
}

template<typename T>
void add_element(matrix<T>& mat, T v)
{
    (mat.data).push_back(v);
}
