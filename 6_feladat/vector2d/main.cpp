...
int main(int, char**) 
{
    ...
    {
        vector2d<double>v{2.1, 23};
        std::string str{"2.1,23"};
        std::stringstream ss;
        ss << v;
        if (ss.str() != str){ std::cout << "Hibás << operátor";};
    }

    {
        vector2d<double>v{2.1, 23};
        std::stringstream ss{"2.1,23"};
        vector2d<double> m;
        ss >> m;
        if (m.x != v.x || m.y != v.y){ std::cout << "Hibás >> operátor";};
    }
} 